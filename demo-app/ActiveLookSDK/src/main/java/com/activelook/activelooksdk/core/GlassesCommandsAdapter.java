...
public class GlassesCommandsAdapter implements GlassesCommands {
    ...
    // Firmware 1.8 static final byte ID_layoutList = (byte) 0x64;
    ...
    // Firmware 1.8 static final byte ID_pageGet = (byte) 0x81;
    static final byte ID_pageDelete = (byte) 0x82;
    // Firmware 1.8 static final byte ID_pageDisplay = (byte) 0x83;
    // Firmware 1.8 static final byte ID_pageClear = (byte) 0x84;
     ...
     */
    // Firmware 1.8 static final byte ID_cfgWrite = (byte) 0xD0;
    // Firmware 1.8 static final byte ID_cfgRead = (byte) 0xD1;
    // Firmware 1.8 static final byte ID_cfgSet = (byte) 0xD2;
    // Firmware 1.8 static final byte ID_cfgList = (byte) 0xD3;
    // Firmware 1.8 static final byte ID_cfgRename = (byte) 0xD4;
    // Firmware 1.8 static final byte ID_cfgDelete = (byte) 0xD5;
    // Firmware 1.8 static final byte ID_cfgDeleteOldest = (byte) 0xD6;
    // Firmware 1.8 static final byte ID_cfgFreeSpace = (byte) 0xD7;
    // Firmware 1.8 static final byte ID_cfgGetNb = (byte) 0xD8;
    ...
    public void shift(short x, short y) {
        this.write(new Payload(ID_shift));
    }
    ...
    public void txt(short x, short y, Rotation r, byte f, byte c, String s) {
        this.write(new Payload(ID_txt).addData(x).addData(y).addData(r.toBytes()).addData(f).addData(s));
    }
    ...
    @Override
    public void imgSave(ImageData data) {
        this.write(new Payload(ID_imgSave).addData(data.getSize()).addData(data.getWidth()));
        for (byte[] chunk : data.getChunks(256)) {
    ...
    public void imgStream(Image1bppData data, short x, short y) {
        this.write(new Payload(ID_imgStream).addData(data.getSize()).addData(data.getWidth()).addData(x).addData(y));
        for (byte[] chunk : data.getChunks(256)) {
    ...
    public void imgSave1bpp(Image1bppData data) {
        this.write(new Payload(ID_imgSave1bpp).addData(data.getSize()).addData(data.getWidth()));
        for (byte[] chunk : data.getChunks(256)) {
    ...
    public void fontSave(byte id, FontData data) {
        this.write(new Payload(ID_fontSave).addData(id).addData(data.getFontSize()));
        for (byte[] chunk : data.getChunks(256)) {
    ...
    public void layoutDisplay(byte id, String text) {
        this.write(new Payload(ID_layoutDisplay).addData(id).addData(text));
    }
    ...
    public void layoutDisplayExtended(byte id, short x, byte y, String text) {
        this.write(new Payload(ID_layoutDisplayExtended).addData(id).addData(x).addData(y).addData(text));
    }
    ...
    @Override
    public void gaugeSave(byte id, short x, short y, short r, short rin, byte start, byte end, boolean clockwise) {
    ...
    @Override
    public void pageSave(byte id, Rotation r, byte[] layoutIds) {
        this.write(new Payload(ID_pageSave).addData(id).addData(r.toBytes()).addData(layoutIds));
    }

    @Override
    public void pageGet(Consumer<List<PageInfo>> onResult) {
        this.write(new Payload(ID_pageList), bytes -> onResult.accept(PageInfo.toList(bytes)));
    }
