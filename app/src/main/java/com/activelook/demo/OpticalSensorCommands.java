...
public class OpticalSensorCommands extends MainActivity2 {
    ...
    protected Map.Entry<String, Consumer<Glasses>>[] getCommands() {
        return new Map.Entry[]{
                ...
                item("setSensorParameters(ALS_PERIOD, 3000)", glasses -> {
                    SensorParameters sp = new SensorParameters();
                    sp.setAlsPeriod((short) 3000);
                    glasses.setSensorParameters(SensorMode.ALS_PERIOD, sp);
                }),
                item("setSensorParameters(ALS_PERIOD, 9000)", glasses -> {
                    SensorParameters sp = new SensorParameters();
                    sp.setAlsPeriod((short) 9000);
                    glasses.setSensorParameters(SensorMode.ALS_PERIOD, sp);
                }),
                item("setSensorParameters(GESTURE_PERIOD, 3000)", glasses -> {
                    SensorParameters sp = new SensorParameters();
                    sp.setGesturePeriod((short) 3000);
                    glasses.setSensorParameters(SensorMode.GESTURE_PERIOD, sp);
                }),
                item("setSensorParameters(GESTURE_PERIOD, 9000)", glasses -> {
                    SensorParameters sp = new SensorParameters();
                    sp.setGesturePeriod((short) 9000);
                    glasses.setSensorParameters(SensorMode.GESTURE_PERIOD, sp);
                }),
